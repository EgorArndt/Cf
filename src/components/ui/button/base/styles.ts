...
export const StyledBase = styled.button<Partial<ButtonBaseProps> & Additional>`
  &&& {
    ...
    ${({ colorOnHover, theme }) =>
      ...
      css`
        &:hover {
          color: ${theme.mutatable.textColors[
            colorOnHover as keyof AppTheme['mutatable']['textColors']
          ] ||
          theme.readonly.commonColors[
            colorOnHover as keyof AppTheme['readonly']['commonColors']
          ].bg} !important;
        }
      `}
